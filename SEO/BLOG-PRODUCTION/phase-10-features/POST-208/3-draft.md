...
- Three main types: conditional (when busy/unanswered), sequential (one-by-one), simultaneous (all at once)
- Time-based routing automatically forwards calls based on business hours without manual intervention
- Failover configuration ensures backup routing when your primary line fails or is unreachable
- Modern AI + forwarding hybrids screen calls intelligently before transferring, reducing interruptions by 70%
- Proper setup takes minutes but prevents $21,700/month in lost revenue for typical contractors
...
[IMAGE: Business owner reviewing call forwarding settings on mobile phone while on job site]

## Types of Call Forwarding: Conditional vs Unconditional

There are two main categories of [call forwarding](https://www.nextiva.com/blog/business-call-forwarding.html): unconditional and conditional. Understanding the difference helps you set up the right system for your business.
...
[Conditional call forwarding](https://www.quo.com/blog/conditional-call-forwarding/) only delivers calls that meet predetermined criteria. Instead of forwarding everything, it routes calls based on your status or specific conditions.
...
[Sequential ringing](https://www.unitedworldtelecom.com/tech/uses-of-sequential-simultaneous-call-forwarding/) forwards calls to numbers one at a time in a preset order. Your business line rings first. If unanswered after 15-30 seconds, it forwards to your mobile. Still no answer? It rings your office phone. Finally, it goes to voicemail.
...
[Simultaneous ringing](https://www.ringover.com/blog/simultaneous-ring) minimizes unanswered calls and reduces response time. Instead of waiting for three sequential 20-second ring cycles, all phones ring immediately. First available person picks up.
...
[Time-based routing](https://www.globalcallforwarding.com/features/time-of-day-routing/) automatically changes your forwarding rules based on schedule. You set it once, and it handles the rest.
...
- If unanswered: Transfer to voicemail

**Monday-Friday, 6 PM - 8 AM:**

- Forward directly to on-call mobile
- Ring time: 25 seconds
- If unanswered: Voicemail with "leave message for callback"

**Saturday:**

- Forward to mobile
- Ring time: 15 seconds
- If unanswered: Voicemail

**Sunday:**

- All calls to voicemail
- Emergency option: "Press 1 to reach on-call technician"
...
[IMAGE: Time-based routing schedule diagram showing business hours (office), after-hours (mobile), and weekends (voicemail)]
...
[Failover forwarding](https://www.unitedworldtelecom.com/features/failover-forwarding/) automatically routes calls to a backup number if the primary destination is unreachable, busy, or rejected. It's your safety net.
...
1. **Primary:** Office VoIP line
2. **Secondary:** Owner's mobile
3. **Tertiary:** Partner's mobile
4. **Final:** Voicemail with callback number
...
[Ring time matters](https://wiki.voip.ms/article/Call_Forwarding). If your forwarded number rings too long, callers hang up before anyone answers.
...
- Callback requests
- Spam filtering (7% of calls eliminated)

**What transfers to you:**

- Emergency situations (15.9% of calls)
- Complex technical questions
- Sales opportunities over certain value
...
**URL Slug:** `/blog/call-forwarding-setup`
